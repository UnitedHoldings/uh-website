  ...
  useEffect(() => {
    const fetchTeamData = async () => {
      try {
        ...
        const response = await fetch('http://uh-server-staging-688256516165.asia-east1.run.app/api/profile');
      ...
      <div className="relative h-[500px] overflow-hidden">
        ...
        {activeCard !== member._id && (
          <div className="absolute z-30 bottom-[10%] left-0 right-0 text-center px-4">
            ...
            <p className="text-lg text-white text-center max-w-[15rem] mx-auto">{member.title}</p>
          </div>
        )}
      </div>
